...
export function DepositModal({ open, onClose }: DepositModalProps) {
  ...
  const bankName = user?.bank_name?.trim() || "-";
      ...
      footer={
        <>
          <Button type="button"
            ...
            onClick={handleSubmit} disabled={isSubmitting || amount < 30000}
          >
            {isSubmitting ? "처리중..." : "입금신청"}
          </Button>
    ...
    >
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5 scrollbar">
          ...
          <div className="border border-[#22242626] rounded-xl bg-white overflow-hidden">
            <table className="w-full text-sm">
              <tbody>
                ...
                <tr className={rowBorder}>
                  <td className="w-32.5 border-r border-[#22242626] bg-white px-4 py-3 text-black">은행명</td>
                  <td className="px-4 py-3 text-left text-black font-bold">{bankName}</td>
                </tr>
                <tr className={rowBorder}>
                  <td className="w-32.5 border-r border-[#22242626] bg-white px-4 py-3 text-black">입금자명</td>
                  <td className="px-4 py-3 text-left text-black font-bold">
                    {user?.bank_holder || user?.userName || "-"}
                  </td>
                </tr>
                <tr className={rowBorder}>
                  <td className="w-32.5 border-r border-[#22242626] bg-white px-4 py-3 text-black">충전금액</td>
                  ...
                  <td colSpan={2} className="px-4 py-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button type="button" variant="transparent"
                        className="px-3 py-1.5 text-xs font-bold bg-white text-black/60 hover:text-black/80 border border-black/60 hover:border-black/80"
                        onClick={handleReset}>
                        금액정정
                      </Button>
        ...
        <div className="border border-[#22242626] rounded-xl bg-white text-sm text-black p-4">
          <h3 className="mb-3 text-lg font-bold text-gold-deep">충전시 주의사항</h3>
        ...
        <div className="border border-[#22242626] rounded-xl bg-white overflow-hidden">
          <div className="border-b border-[#22242626] px-4 py-3 text-lg font-bold text-gold-deep">입금내역</div>
